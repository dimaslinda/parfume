                ...
                @if (session('success'))
                    <div class="p-4 mb-4 text-white bg-green-500 rounded-lg shadow-md" role="alert">
                        <div class="flex justify-between items-center">
                            ...
                            <button type="button" class="ml-2 text-white" onclick="this.parentElement.parentElement.style.display='none';">&times;</button>
                        </div>
                    </div>
                @endif
                    ...
                    <div class="flex flex-row space-x-5 w-full md:flex-col md:px-10">
                        <div class="flex relative justify-center mb-10">
                            <div class="overflow-hidden relative w-40 h-40 rounded-full border-4 border-[#6298CE]">
                                <img src="{{$user->getFirstMediaUrl('avatars') ?: asset('img/general/default.jpg')}}" class="object-cover w-full h-full" alt="profile">
                            </div>
                    ...
                    <div id="default-styled-tab-content" class="border-l border-[#c2c2c2] p-6">
                            ...
                            <div class="border-2 border-[#c2c2c2] p-6 rounded-lg bg-white flex flex-col">
                                ...
                                @if(isset($claims[$item->id]))
                                    @if($claims[$item->id]->status == 'approved')
                                        <span class="px-5 py-2 w-full font-bold text-center text-white uppercase bg-green-500 cursor-not-allowed lg:text-xl">
                                            Claimed
                                        </span>
                                    @elseif($claims[$item->id]->status == 'pending')
                                        <span class="px-5 py-2 w-full font-bold text-center text-white uppercase bg-yellow-500 cursor-not-allowed lg:text-xl">
                                            Pending
                                        </span>
                                    @elseif($claims[$item->id]->status == 'rejected')
                                        <span class="px-5 py-2 w-full font-bold text-center text-white uppercase bg-red-500 cursor-not-allowed lg:text-xl">
                                            Rejected
                                        </span>
                                    @endif
                                @elseif(isset($user->points) && $user->points->points >= $item->point_cost)
                                    <form method="POST" action="{{ route('claim-reward', $item->id) }}" class="w-full">
                                        @csrf
                                        <button type="submit" class="px-5 py-2 w-full font-bold text-center text-white uppercase lg:text-xl bg-tombol">
                                            Claim
                                        </button>
                                    </form>
                                @else
                                    <span class="px-5 py-2 w-full lg:text-xl font-bold text-center text-white uppercase bg-[#BEBEBE]">
                                        locked
                                    </span>
                                @endif
                                </div>
                        ...
                        <div class="hidden overflow-y-auto flex-col p-4 space-y-5 h-96 rounded-lg" id="styled-dashboard" role="tabpanel" aria-labelledby="dashboard-tab">
                            ...
                            @foreach ($users->take(10) as $index => $user)
                                @php
                                    $rank = $index + 1;
                                    $bgColor = $rank == 1 ? 'bg-tombol' : ($rank == 2 ? 'bg-[#6298CE]' : ($rank == 3 ? 'bg-[#D6EBFF]' : 'bg-white'));
                                    $textColor = $rank <= 2 ? 'text-white' :($rank == 3 ? 'text-[#6298CE]' : 'text-black');
                                    $borderColor = $rank <= 3 ? 'border-2' : 'border';
                                @endphp
                                <div class="flex flex-col gap-3 p-6 {{ $textColor }} rounded-lg {{ $borderColor }} md:gap-0 md:flex-row {{ $bgColor }}">
                                    ...
                                    <div class="flex flex-row flex-1 justify-end items-center self-center space-x-5">
                                        ...
                                        <div class="overflow-hidden w-10 h-10 rounded-full md:w-20 md:h-20">
                                            <img src="{{ asset("img/tier/{$user->getTierLogo()}") }}" class="object-cover w-full h-full" alt="tier">
                                        </div>
                                    </div>
                                </div>
                                @endforeach
